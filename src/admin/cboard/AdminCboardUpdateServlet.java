 ...
 */
@WebServlet("/adcupdate.ad")
public class AdminCboardUpdateServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    /**
	 * @see HttpServlet#HttpServlet()
	 */
    public AdminCboardUpdateServlet() {
        super();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
        ...
        for (int i = 0; i < 4; i++) {
            deleteFlag = mrequest.getParameter("delflag" + (
                i + 1
            ));
            originFilePath = mrequest.getParameter("ofile" + (
                i + 1
            ));
            renameFilePath = mrequest.getParameter("rfile" + (
                i + 1
            ));
            ...
            if (originalFileName != null) {
                ...
                renameFileName += i + "." + originFilePath.substring(
                ...
                originFile.delete();
            } else if (originFilePath != null && (originalFileName == null || originFilePath.equals(originalFileName) && originFilePath.contentEquals(originalFileName) && newOriginFile.length() == originFile.length())) {
                switch (i + 1) {
                    case 1:
        ...
        if (result > 0) {
            ...
            writer.println(
                "<script>alert('글수정 실패');location.href='/anavada/adcdetail.ad?cnum=" + cboard.getCboardNo() +
                "';</script>"
            );
            writer.close();
        }
