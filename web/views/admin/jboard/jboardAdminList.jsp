<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" import="jboard.model.vo.Jboard"%>
<%
		...
		<div id="admin_container">
			...
			<div class="list-area">
				...
				<div class="sort-area">
					...
					<div>
						<form action="/anavada/adjlistview.ad" method="get" style="display: inline-block;">
							지역 분류 :
				...
				<form>
					...
					<table class="jboardTable" style="width: inherit;" co>
						...
						<tbody>
							...
							<%for(Jboard j : jlist) {%>
							<tr onclick="location.href='/anavada/jbdetail?jboardno=<%= j.getJboardNo()%>'">
								<td class="checkBox" onclick="event.cancelBubble=true">
									<input type="checkbox" name="check" value="<%= j.getJboardNo()%>">
								</td>
								...
								<td class="jboardupdate">
									<% if(j.getJboardUpdate() != null) { %>
									<%= j.getJboardUpdate() %></td>
								<% } %>
							</tr>
							<%
                       		}
					...
					<div class="btn_wrap">
						...
						<a href="views/jboard/product_write.jsp" class="btn-right btn_white">등록</a>
					</div>
				</form>
...
<dl class="list-paging pb80">
                <dd>
                	<% if (currentPage <= 1){ %>
              		...
              		<%}else {%>
              		<a href="/anavada/adjlistview.ad?local=<%=local%>&titleserch=<%=titleSearch%>&listsearch=<%=listSearch%>"><i class="glyphicon glyphicon-backward"></i></a>
              		<%} %>
              		<% if (startPage -1 >=10 ){ %>
                    <a href="/anavada/adjlistview.ad?page=<%=startPage - 10%>&local=<%=local%>&listsearch=<%=listSearch%>&titlesearch=<%=titleSearch%>"> <i class="glyphicon glyphicon-menu-left"></i>
					<%} %>
                   ...
                   <a href="/anavada/adjlistview.ad?page=<%=p%>&local=<%=local%>&listsearch=<%=listSearch%>&titlesearch=<%=titleSearch%>"><%=p%></a>
                    ...
                    <%}else{ %>
                    <a href="/anavada/adjlistview.ad?page=<%=maxPage%>&local=<%=local%>&listsearch=<%=listSearch%>&titlesearch=<%=titleSearch%>"><i class="glyphicon glyphicon-forward"></i></a>
                    <%} %>
                </dd>
            </dl>
				<!-- //페이징 -->
